...
public class Banner extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        if (changed || forceToRelayout) {
            ...
            if (mCurrentIndex >= 0 && mCurrentIndex < mChildCount) {
                ...
                setScrollX(0);
                int left = -childWidth * 2;
                updateItems();
                // 通过循环为每张图片设定位置  
                for (int i = 0; i < mChildCount; i++) {
                    View childView =  getChildAt(mItems[i]);
    ...
    private void updateItems() {
        ...
        for (int i = 1; i <= mChildCount; i++) {
            mItems[i-1]=getIndexForItem(i);
            Log.e(TAG,"mItems["+i+"] = " +mItems[i-1]);
        }
    }
